			...
			if (timetest > 60000) {
				...
				if (client.users.cache.get(botdata.ownerID)) {
					client.channels.cache
						.get(config.server.channels.downtimelog)
						.send(`<@${botdata.ownerID}>`, emb);
				} else {
					client.channels.cache.get(config.server.channels.downtimelog).send(emb);
				}
			}
			await botsdata.findOneAndUpdate(
				{
					botID: newPresence.userID
				},
				{
					$set: {
			...
			if (rnxd.votes === target) {
				...
				client.channels.cache
					...
					.send(
						`:tada: :tada: <@${
							rnxd.botID
						}> has Reached the Vote Target of ${target} Owner of Bot: <@${
							rnxd.ownerID
						}> :tada: :tada:`
					);
			}
...
client.on('ready', async () => {
	...
	client.user.setPresence({
		activity: {
			type: 'WATCHING',
			name: 'bhbotlist.xyz | ' + bots.length + ' bots'
		},
		status: 'dnd'
	});
});
